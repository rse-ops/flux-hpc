...
parser.add_argument("--workers", type=int, help="Number of workers (defaults to cpu count)", default=os.cpu_count())
parser.add_argument("--search-space", help="search space of molecules (selected randomly from the QM9 database)", default='data/QM9-search.tsv')
parser.add_argument("--initial-count", type=int, help="Number of calculations to run at first", default=8)
parser.add_argument("--search-count", type=int, help="Number of molecules to evaluate in total", default=64)
parser.add_argument("--batch-size", type=int, help="Number of molecules to evaluate in each batch of simulations", default=4)
parser.add_argument("--hostname", help="Hostname for task columna server", default='localhost')
parser.add_argument("--outdir", help="Output directory (defaults PWD if not defined)", default=os.getcwd())
parser.add_argument("--working-dir", help="Working directory for Flux (defaults to PWD)", default=os.getcwd())
parser.add_argument("--flux-workers", type=int, help="Number of flux node available to run", default=4)
...
search_space = pd.read_csv(args.search_space, delim_whitespace=True)  # Our search space of molecules
