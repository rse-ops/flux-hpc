...
n_workers = max(
    len(os.sched_getaffinity(0)) - 1, 1
)  # Get as many threads as we are assigned to
_pool = ProcessPoolExecutor(max_workers=n_workers)

"""SIMULATION FUNCTIONS: Quantum chemistry parts of the workflow"""

_spec = {"model": {"method": "GFN2-xTB", "basis": None}, "keywords": {"accuracy": 0.05}}
...
def _compute_vertical(smiles: str) -> float:
    ...
    opt_input = OptimizationInput(
        input_specification=_spec, initial_molecule=mol, keywords={"program": "xtb"}
    )
    opt_res = compute_procedure(opt_input, "geometric", raise_error=True)

    # Compute the energy of the relaxed geometry in charged form
    charged_mol = Molecule.from_data(
        opt_res.final_molecule.to_string("xyz"), molecular_charge=1
    )
